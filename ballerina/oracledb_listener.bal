...
# Represents a Ballerina CDC Oracle DB Listener.
public isolated class OracleListener {

    private final map<string> & readonly config;
    private boolean isStarted = false;
    private boolean hasAttachedService = false;

    # Initializes the Oracle DB listener with the given configuration.
    #
    # + config - The configuration for the Oracle DB connector
    public isolated function init(*OracleListenerConfiguration config) {
    ...
    # Stops the Oracle DB listener gracefully.
    #
    ...
    # Stops the Oracle DB listener immediately.
    #
    # + return - An error if the listener cannot be stopped, or `()` if successful
